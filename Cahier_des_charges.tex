...
\begin{document}
   ...
   \begin{itemize}
      \item Une grille 2D (par défaut de dimension 100×100) composée de cellules.
      \item des cellules donc (structure) qui sont composées de 2 triangles afin de procéder au rendu. ainsi qu'une ribambelle de paramètres et méthodes (coordonnées dans la grille, température, vecteur, pression,est-ce une cellule au bord de la grille... etc).
      \item Un fenêtre d'affichage graphique simple permettant de visualiser (avec OpenGL).
      \item Un simulation de génération aléatoires de poinds chauds avec une remise à équilibre termique via un simple calcul de moyenne afin de soumettre tous les éléments cités précédemment à l'épreuve. 
   \end{itemize}
